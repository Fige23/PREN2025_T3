...
typedef struct {
    ...
    uint16_t profile_scale_pct[MOTION_PROFILE_KIND_COUNT];
} motion_tuning_state_s;

static motion_tuning_state_s g_motion_tuning;

static bool kind_valid(motion_profile_kind_e kind){
    return (kind >= MOTION_PROFILE_KIND_MOVE) && (kind < MOTION_PROFILE_KIND_COUNT);
}
...
void motion_tuning_init(void){
    ...
    for(i = 0; i < (uint32_t)MOTION_PROFILE_KIND_COUNT; ++i){
...
uint16_t motion_tuning_get_global_scale_pct(void){
    if(g_motion_tuning.global_scale_pct == 0u){
        return 100u;
    }
...
uint16_t motion_tuning_get_profile_scale_pct(motion_profile_kind_e kind){
    if(!kind_valid(kind) || g_motion_tuning.profile_scale_pct[(uint32_t)kind] == 0u){
        return 100u;
    }
