...
typedef struct {
	const char *key_lc; //"x","y","z","phi"
	int32_t *dst_scaled; //Ziel (zb 0.001mm)
	int32_t min_scaled;
	int32_t max_scaled;
	uint8_t bit;
	int32_t scale;
...
err_e kv_fixed_any_lower(const char *tok, const kv_fixed_spec_s *specs, size_t n, uint8_t *seen_mask);

// Parst argv[start..argc-1] für x,y,z,phi in Fixed-Point.
// require_mask: Keys, die zwingend vorkommen müssen (z.B. KV_X|KV_Y|KV_Z)
...
err_e parse_pos_tokens_mask(int argc, char **argv, int start, int32_t *x_s, int32_t *y_s, int32_t *z_s, int32_t *ph_s, uint8_t require_mask, uint8_t allowed_mask, uint8_t *seen_out);
