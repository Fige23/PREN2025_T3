...
void ftm3_init(void)
{
  // todo ML#09.01 set clockgating for FTM3
  SIM->SCGC6 |= SIM_SCGC6_FTM3(1);

  // sets the modulo
  FTM3->MOD = FTM3_MODULO;

  // todo ML#09.02 configure the timer with "system clock" as clocksource and with a "Prescaler" of 1 => 60 MHz
  FTM3->SC = FTM_SC_CLKS(1) |  FTM_SC_PS(0);

  // todo ML#09.03 Enable FTM3 interrupt on NVIC with Prio: PRIO_FTM3 (defined in platform.h)
